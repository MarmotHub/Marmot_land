...
describe('MarmotPool', function () {
    ...
    before(async function() {
        ...
        curBlockNumber = await pool.blockNumber()
        console.log("blockNumber at deploy", curBlockNumber.toString())
        await pool.initialize(
            marmot.address,
            decimalStr("10"),
            decimalStr("15"),
            curBlockNumber.toNumber() + 40
        )
    ...
    //  it('marmot lastRewardBlock', async function () {
    //     await console.log('curblockNumber', (await pool.blockNumber()).toString())
    //     await pool.connect(deployer).deposit(0, decimalStr("30"))
    //     await pool.connect(deployer).deposit(1, decimalStr("0.005"))
    //     await pool.connect(alice).deposit(0, decimalStr("50"))
    //     await console.log('lastRewardBlock', (await pool.lastRewardBlock()).toString())
    //     await console.log('curblockNumber', (await pool.blockNumber()).toString())
    //
    //      console.log('deployer pendingAll', await pool.pendingAll())
    //      console.log('alice pendingAll', await pool.connect(alice).pendingAll())
    //      fastMove(100)
    //      // await new Promise(resolve => setTimeout(resolve, 60000))
    //
    //      await busd.connect(deployer).transfer(bob.address, decimalStr("1"))
    //      await busd.connect(deployer).transfer(bob.address, decimalStr("1"))
    //      await busd.connect(deployer).transfer(bob.address, decimalStr("1"))
    //      await busd.connect(deployer).transfer(bob.address, decimalStr("1"))
    //
    //      await console.log('curblockNumber', (await pool.blockNumber()).toString())
    //     console.log('deployer pendingAll', (await pool.pendingAll()).toString())
    //      console.log('alice pendingAll', (await pool.connect(alice).pendingAll()).toString())
    //
    //      await pool.connect(alice).claimAll();
    //     console.log('alice marmot balance', (await marmot.balanceOf(alice.address)).toString())
    //
    // })
    ...
    it('update pool', async function () {
        console.log('pool info', await pool.getPoolInfo(0))
        console.log('lastReward', (await pool.lastRewardBlock()).toString())
        await pool.setPool(0, busd.address, decimalStr("0.1"),
            ZERO_ADDRESS,
            "0xe5ed8148fE4915cE857FC648b9BdEF8Bb9491Fa5",
            3)
        console.log('lastReward', (await pool.lastRewardBlock()).toString())
